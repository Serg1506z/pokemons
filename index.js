...
const ShowInnerPokemonPage = async ({ id }) => {
  ...
  const personalCardPokemonImage = document.querySelector(
    "#personalCardPokemonImage"
  );
  personalCardPokemonImage.src =
    pokemon.sprites.other["official-artwork"]["front_default"];

  const personalCardPokemonName = document.querySelector(
    "#personalCardPokemonName"
  );
  personalCardPokemonName.innerHTML = pokemon.name;

  const personalCardPokemonId = document.querySelector(
    "#personalCardPokemonId"
  );
...
const getAllPokemons = async (offset) => {
  ...
  const response = await fetch(
    `${POKEMON_API}/pokemon?limit=${POKEMON_LIMIT}&offset=${offset}`,
    { method: "GET" }
  );
  ...
  const pokemonsData = await Promise.allSettled(
